...
async function getHudData(ns, bitNode, dictSourceFiles, options) {
    ...
    // Show Hashes
    {
        ...
        if (9 in dictSourceFiles || 9 == bitNode) { // Section not relevant if you don't have access to hacknet servers
            const hashes = await getNsDataThroughFile(ns, '[ns.hacknet.numHashes(), ns.hacknet.hashCapacity()]', '/tmp/insight/hash-stats.txt');
            if (hashes[1] > 0) {
                ...
                const liquidatingHashes = await getNsDataThroughFile(ns,
                    ...
                    '/tmp/insight/hash-liquidation-scripts.txt', [spendHashesScript]);
...
async function getGangInfo(ns) {
    return await getNsDataThroughFile(ns, 'ns.gang.inGang() ? ns.gang.getGangInformation() : false', '/tmp/insight/gang-stats.txt');
}
...
async function getAllServersInfo(ns) {
    ...
    return await getNsDataThroughFile(ns, 'ns.args.map(ns.getServer)', '/tmp/insight/getServers.txt', serverNames);
}
